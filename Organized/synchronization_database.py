...
class ExperimentWindow:
    def __init__(self, master, db_obj):
        ...
        for patient in all_patients:
            ...
            b = Button(self.frame, text=patient[0] + " - " + patient[1],
                       command=lambda index=patient[0]: self.select_files(index), font=self.text_font)
    ...
    def select_files(self, index):
        ...
        self.button_insert_experiment = Button(self.frame, text="Insert on Database",
                                               command=lambda: self.insert_experiment(session_filename,
                                                                                      timings_filename, index),
                                               font=self.text_font)
        self.button_insert_experiment.grid(row=3, column=0)

        self.button_show_experiments = Button(self.frame, text="Show experiments",
                                              command=lambda: self.show_experiments(index), font=self.text_font)
    ...
    def insert_experiment(self, session_name, timings_name, patient_id):
        ...
        experiment_dest_folder = "C:\\Users\\Naim\\Desktop\\Tese\\Programming\\Data\\" + patient_id + "\\" + repr(
            experiment_number)
